...
def move(data):
    ...
    nearby = data['nearby']
    directions_to_check = [(direction + 1) % 4, (4 + direction - 1) % 4, direction]

    for new_direction in directions_to_check:
        if nearby[mp[new_direction][0]][mp[new_direction][1]] != 0:
            direction = new_direction
            break

    action = dir_str[direction]
